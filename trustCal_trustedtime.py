...
class UserInfo(Document):
    ...
    def tweets_timer_trustedtime(self, tweets):
        trusted = []
        for tweet in tweets:
            timeObj = re.match('.*?(\d{2,}):(\d{2,})', tweet.PubTime)
            hour = int(timeObj.group(1))
            ...
            for center in centers:
                if abs(center[0] - timeset) < dis:
                    dis = abs(center[0] - timeset)
            print(dis)
            if dis > TIMELIMIT:
                trusted.append(1)
            else:
...
if __name__ == '__main__':
    ...
    for user in UserInfo.objects[:1]:
        ...
        Trust_value.write('\n')
        '''
        最后4条微博作为测试集判断是否为异常点，暂时不考虑kmeans重新计算
       '''
        tweets = user.get_tweets()
        kmeans = user.tweets_timer_cluster(tweets[0:len(tweets)-5])
        labels_ = kmeans.labels_.tolist()
        cluster_centers_ = kmeans.cluster_centers_.tolist()
        print(kmeans.labels_)
        print(kmeans.cluster_centers_)
        print(user.tweets_timer)
        user.tweets_timer_trustedtime(tweets[len(tweets)-4:])
